"""
Header Analysis Module - Excellent Edition (Realistic Scoring)
"""
...
class HeaderAnalyzer:
    def __init__(self, config: Dict):
        ...
        # New: headers that are nice-to-have but not security-critical if missing
    ...
    def _check_missing_headers(self, headers: Dict, analysis: Dict):
        ...
        for header in crit:
            if header.lower() not in headers:
                ...
                # Downgrade HSTS if HTTPS and likely preloaded
                if header == 'Strict-Transport-Security' and analysis['headers_found'].get('_status_code','') == '200' and analysis['url'].startswith('https://'):
                    severity = 'high'
        ...
        for header in high:
            if header.lower() not in headers:
                ...
                # X-Content-Type-Options is less critical if Content-Type is set
        ...
        for header in med:
            if header.lower() not in headers:
                # Referrer-Policy and Permissions-Policy are optional; downgrade to recommendation
                if header in self.optional_headers:
                    # Don't add to missing_required; only to recommendations later
                    continue
                analysis['missing_required'].append(header)
                analysis['vulnerabilities'].append({
                    'type': 'missing_medium',
    ...
    def _check_vulnerable_headers(self, headers: Dict, analysis: Dict):
        for header in self.vulnerable_headers:
            if header.lower() in headers:
                ...
                # Only flag Server if it contains a version pattern
                if header == 'Server' and not re.search(r'\d+\.\d+', value):
                    continue  # ignore non-version server tokens
                analysis['vulnerabilities'].append({
    ...
    # ---------- Header-specific analyses (simplified, already correct) ----------
    def _analyze_hsts(self, headers, analysis, url):
        hsts = headers.get('strict-transport-security')
        if not hsts:
            return
        analysis['detailed_analysis']['hsts'] = {'header': hsts, 'max_age': None, 'include_subdomains': False, 'preload': False, 'issues': []}
    ...
    def _analyze_csp(self, headers, analysis):
        ...
        # CSP missing but report-only present: give partial credit
    ...
    def _analyze_cookies(self, headers, analysis):
        ...
        for cookie in set_cookies:
            ...
            if 'httponly' in cl: ca['flags']['httponly'] = True
            ...
            if samesite_match:
                ...
                if required_level > 0:  # Lax or Strict required
    ...
    # (Other analysis methods unchanged: _analyze_cors, _analyze_cache_control, etc.)
    # (Include them from the previous version, they don't need changes)

    # For brevity, I'll skip pasting all unchanged methods. They remain as in the previous excellent edition,
    # except we ensure _analyze_referrer_policy and _analyze_permissions_policy are still present but they aren't
    # critical for the scoring fix.
    ...
    def _check_server_banner(self, headers, analysis):
        ...
        if server and re.search(r'\d+\.\d+', server):   # only if version number pattern
    ...
    # ---------- Scoring with nuanced deductions ----------
    def _calculate_security_score(self, analysis: Dict):
        ...
        score += crit * gc.get('missing_critical', -10)   # usually -10
        score += high * gc.get('missing_high', -7)        # -7
        score += med  * gc.get('missing_medium', -4)      # -4
        score += low  * gc.get('weak_configuration', -5)  # treat low as weak_config

        # Additional penalties from grading config
        ...
        # Bonus for present required headers
        ...
        # Bonus for strong HSTS (if present and good)
        ...
        # If only CSP-Report-Only exists, give a smaller bonus
        if not csp and analysis['headers_found'].get('content-security-policy-report-only'):
    ...
    def _generate_recommendations(self, analysis: Dict) -> List[str]:
        ...
        for v in analysis['vulnerabilities']:
            if v['type'] in ('missing_critical','missing_high','missing_medium'):
            ...
            elif v['type'].startswith('weak_') or v['type'] in ('csp_report_only',):
                recs.append(f"MEDIUM: Strengthen {v['header']} – {v['description']}")
            # Don't generate recs for info/infrastructure
        # Add missing recommended headers as LOW priority
        for h in analysis['missing_recommended']:
            recs.append(f"LOW: Consider adding recommended header {h}")
        # Also suggest Referrer-Policy and Permissions-Policy if missing but not already mentioned
