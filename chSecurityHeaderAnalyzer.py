...
class ChSecurityHeaderAnalyzer:
    def __init__(self):
        ...
        self.scanner = SecurityScanner(self.config, self.logger)
    ...
    def run(self):
        ...
        parser.add_argument('--version', action='version',
                            version='ChSecurityHeaderAnalyzer 2.0.0')
        ...
        if args.no_verify:
            self.config['scanner']['verify_ssl'] = False
        if args.no_redirect:
            self.config['scanner']['follow_redirects'] = False
        if args.user_agent:
            self.config['scanner']['user_agent'] = args.user_agent
        if args.threads:
            self.config['scanner']['thread_count'] = args.threads

        output_formats = None if args.no_report else args.output
        if output_formats and 'all' in output_formats:
        ...
        try:
            ...
            if output_formats:
                ...
                for fmt, path in report_paths.items():
                    self.ui.print_success(f"{fmt.upper()} report saved: {path}")
    ...
    def analyze_multiple(self, urls: List[str], output_formats: List[str] = None,
        ...
        with ThreadPoolExecutor(max_workers=thread_count) as executor:
            ...
            for future in as_completed(future_to_url):
                ...
                try:
                    ...
                    self.logger.error(f"Worker error for {url}", exc_info=True)
        ...
        if len(results) > 1 and output_formats:
            ...
            for fmt, path in combined_paths.items():
                self.ui.print_success(f"Combined {fmt.upper()} report saved: {path}")
    ...
    def _analyze_single_worker(self, url: str, output_formats: List[str] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.error(f"Worker error for {url}: {str(e)}", exc_info=True)

        return None

    def analyze_from_file(self, file_path: str, output_formats: List[str] = None,
                          output_dir: str = None, threads: int = None) -> List[Dict[str, Any]]:
        """Analyze websites from a file"""
        try:
